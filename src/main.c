...
typedef enum { CLOSED = 0, UNKNOWN = 1, OPEN = 2 } state;
...
#define ZEPHYR_GPIO(N)                                                         \
  const struct gpio_dt_spec N = GPIO_DT_SPEC_GET(ZEPHYR_USER_NODE, N##_gpios)
...
void main(void) {
  ...
  if (!(device_is_ready(led.port) && device_is_ready(open_button.gpio.port) &&
